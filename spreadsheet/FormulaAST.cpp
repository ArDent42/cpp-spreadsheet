...
enum ExprPrecedence {
	EP_ADD, EP_SUB, EP_MUL, EP_DIV, EP_UNARY, EP_ATOM, EP_END,
};
...
public:
	...
	virtual double Evaluate(std::function<double(Position)> get_value) const = 0;
...
public:
	...
	double Evaluate(std::function<double(Position)> get_value) const override {
		auto lhs = lhs_->Evaluate(get_value);
		auto rhs = rhs_->Evaluate(get_value);
		double res = 0.0;
		switch (type_) {
		case '+':
			res = lhs + rhs;
...
public:
	...
	double Evaluate(std::function<double(Position)> get_value) const override {
		auto res = operand_->Evaluate(get_value);
		switch (type_) {
		case '+':
			break;
		case '-':
...
double FormulaAST::Execute(std::function<double(Position)> get_value) const {
