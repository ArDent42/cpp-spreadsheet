...
void TestEmptyCellTreatedAsZero() {
	...
	const auto val = sheet->GetCell("A1"_pos)->GetValue();
...
void TestPrint() {
	...
	const auto str1 = texts.str();
	ASSERT_EQUAL(texts.str(), "\t\nmeow\t=35\n");

	std::ostringstream values;
	sheet->PrintValues(values);
	const auto str2 = values.str();
...
void TestSetGetCellCircularDependency() {
    auto sheet = CreateSheet();
    ...
    std::cerr << "TestSetGetCellCircularDependency passed" << std::endl;
}
...
int main() {
	...
	RUN_TEST(tr, Test10);
	TestSetGetCellCircularDependency();
	return 0;
}
